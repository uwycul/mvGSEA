...
#------------------------------------------------------------
 ...
 #------------------------------------------------------
  ...
  #if there are no row names, put in a warning to specify row names
 ...
 #--------------------------------------
  ...
  X_new_ <- X_new[gtools::mixedsort(rownames(X_new), decreasing = FALSE),]
  #for ys, taking out the non-overlapping row names-----
  ys_indices <- pmatch(common,rownames(ys))
  ys_new <- ys[ys_indices,]
  ys_new_ <- ys_new[gtools::mixedsort(rownames(ys_new), decreasing = FALSE),]
  #-----------------------------------------
