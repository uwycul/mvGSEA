...
edited_wrapper_mvGSEA <- function(ys,X, v, conf=NULL, mc=4, prediction_method=c("threshold", "sensitivity", "custom"),prediction_cutoff=0.05)
{
  ...
  } else{ #if there ARE row names (length doesn't equal 0)
    if (all(rownames(ys) == rownames(X))) {
      #if there are rownames and the rownames match up
      X_new_ <- X
      ys_new_ <- ys
